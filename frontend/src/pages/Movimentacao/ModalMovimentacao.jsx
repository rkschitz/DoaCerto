...
export default function MovimentacaoModal({
  ...
  useEffect(() => {
    if (show) {
      if (movimentacaoSelecionada?.idMovimentacao) {
        setMovimentacao({
          ...movimentacaoSelecionada,
          dataMovimentacao: formatarDataInput(
            movimentacaoSelecionada.dataMovimentacao
          ),
  ...
  const salvar = async () => {
    try {
      const response = movimentacaoSelecionada?.idMovimentacao
        ? await editarMovimentacao(movimentacao)
  ...
  useEffect(() => {
    if (!movimentacao?.idMovimentacaoAlimento) {
      ...
      const quantidadeInvalida =
        !movimentacao.quantidade || Number(movimentacao.quantidade) <= 0;
    ...
    >
      <Row className="mb-3">
        <SelectTipoMovimentacao
          onChange={(ieMovimentacao) =>
            setMovimentacao({ ...movimentacao, ieMovimentacao })
          }
      ...
      <Row className="mb-3">
        <SelectCampanha
          onChange={(campanha) =>
            setMovimentacao({ ...movimentacao, idCampanha: campanha })
          }
      ...
      <Row className="mb-3">
        <SelectPessoa
          label="Doador"
          onChange={(doador) =>
            setMovimentacao({ ...movimentacao, idDoador: doador.idPessoa })
          }
          value={movimentacao.nomeDoador}
          disabled={
            !movimentacao.ieMovimentacao || movimentacao.ieMovimentacao === "S"
          }
        />
      </Row>
      <Row className="mb-3">
        <SelectDonatario
          ...
          disabled={
            !movimentacao.ieMovimentacao || movimentacao.ieMovimentacao === "E"
          }
        />
      </Row>
      {!movimentacao?.idMovimentacaoAlimento ? (
          ...
          {movimentacao.alimentos?.map((item, index) => (
            <div key={index}>
              <Row>
                <SelectAlimento
                  onChange={(alimento) => {
                    atualizarAlimento(index, "idAlimento", alimento);
                  }}
                ...
                <Col md={4}>
                  <SelectUnidadeMedida
                    ...
                    onChange={(unidadeMedida) =>
                      atualizarAlimento(index, "idUnidadeMedida", unidadeMedida)
                    }
                  />
                </Col>
                <Col md={3}>
                  <Form.Control
                    ...
                    onChange={(e) =>
                      atualizarAlimento(index, "quantidade", e.target.value)
                    }
                  />
                </Col>
        ...
        <div>
          <Row className="mb-3">
            <SelectAlimento
              onChange={(alimento) =>
                setMovimentacao({ ...movimentacao, idAlimento: alimento })
              }
          ...
          <Row className="mb-3 justify-content-center align-items-end">
            <Form.Group className="col">
              <SelectUnidadeMedida
                value={movimentacao.idUnidadeMedida}
                onChange={(unidadeMedida) =>
                  setMovimentacao({
                    ...movimentacao,
